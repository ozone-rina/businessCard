...
@SpringBootTest
@ActiveProfiles("unit")
public class PersonalinfoServieTests {
	...
	void test01() {
		// 実行
		List<BusinessCardModel> result = service.selectAll();
	...
	void test02() {
		...
		// 実行
		BusinessCardModel result = service.findById(id);
...
//		BusinessCardModel model = new BusinessCardModel();
		...
		// 実行
		int companyresult = companyService.insertSelective(companytable);
		int result = service.insertSelective(expectedTable);

		// 検証
		assertEquals(1, result);
		assertEquals(1, companyresult);
	...
	void test07() {
		...
		// 実行
		List<BusinessCardModel> result = service.mySelectByExample(keyword);
	...
	void test08() {
	 ...
	 // 検証_リスト内のいづれかの要素が部分一致していることを確認
	    for (BusinessCardModel card : result) {
